...
static uint8_t _WriteReg_Retry(uint8_t reg, uint8_t data)
{
    ...
    {
        MyI2C_Start();
        ret = MyI2C_SendByte(PCA9685_ADDR_M << 1);
...
uint8_t PCA9685_M_ReadReg(uint8_t reg)
{
    ...
    MyI2C_Start();
    if (MyI2C_SendByte(PCA9685_ADDR_M << 1) != I2C_ACK_OK) goto exit;
    if (MyI2C_SendByte(reg) != I2C_ACK_OK) goto exit;
    
    MyI2C_Start();
    if (MyI2C_SendByte((PCA9685_ADDR_M << 1) | 1) != I2C_ACK_OK) goto exit;
...
void PCA9685_M_SetAngle(uint8_t channel, uint8_t angle)
{
    ...
    PCA9685_M_SetPWM(channel, 0, pulse);
}
